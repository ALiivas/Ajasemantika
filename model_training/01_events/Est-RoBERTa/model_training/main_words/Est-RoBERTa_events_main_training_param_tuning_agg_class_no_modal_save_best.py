...
X_train, y_train = split_data(train_texts, 'gold_word_events_main_w_agg_classes_no_modal')
X_dev, y_dev = split_data(dev_texts, 'gold_word_events_main_w_agg_classes_no_modal')
X_test, y_test = split_data(test_texts, 'gold_word_events_main_w_agg_classes_no_modal')
...
def compute_metrics(p):
    ...
    eval_nervaluate = Evaluator(true_labels, true_predictions, tags=['EVENT_ARG_CLASS', 'EVENT_OCCURRENCE', 'EVENT_STATE'], loader='list')
...
def perform_param_grid_search():
    ...
    def trial_training(num_train_epochs, per_device_train_batch_size, learning_rate):
        trial_training_args = TrainingArguments(
            output_dir='est-roberta_events_main_w_agg_class_no_modal_trial_best',
            num_train_epochs=num_train_epochs,
            per_device_train_batch_size=per_device_train_batch_size,
            logging_dir='est-roberta_events_main_w_agg_class_no_modal_trial_best',
...
with open('est-roberta_events_main_w_agg_class_no_modal_best_best_params.json', 'w') as f:
...
training_args = TrainingArguments(
    output_dir='est-roberta_events_main_w_agg_class_no_modal_param_tuning_best',
    num_train_epochs=40,
    per_device_train_batch_size=best_parameters['per_device_train_batch_size'],
    logging_dir='est-roberta_events_main_w_agg_class_no_modal_param_tuning_best',
    logging_strategy='epoch',
    save_strategy='epoch',
...
with open('est-roberta_events_main_w_agg_class_no_modal_param_tuning_best_eval.json', 'w') as f:
    json.dump(training_results, f)

# -- Saving model
trainer.save_model("./est-roberta_events_main_w_agg_class_no_modal_param_tuning_best/best/")
...
nervaluate = Evaluator(final_labels, final_predictions, tags=['EVENT_ARG_CLASS', 'EVENT_OCCURRENCE', 'EVENT_STATE'], loader='list')
results, results_by_tag, result_indices, result_indices_by_tag = nervaluate.evaluate()

# -- Saving final test results to JSON
with open('est-roberta_events_main_w_agg_class_no_modal_param_tuning_best_final_test.json', 'w') as f:
