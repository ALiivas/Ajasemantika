...
X_train, y_train = split_data(train_texts, 'gold_word_events_main_w_classes')
X_dev, y_dev = split_data(dev_texts, 'gold_word_events_main_w_classes')
X_test, y_test = split_data(test_texts, 'gold_word_events_main_w_classes')
...
# -- Loading tokenizer
tokenizer = AutoTokenizer.from_pretrained("EMBEDDIA/est-roberta", do_lower_case=False)
...
def compute_metrics(p):
    ...
    eval_nervaluate = Evaluator(true_labels, true_predictions, tags=['EVENT_ASPECTUAL', 'EVENT_OCCURRENCE', 'EVENT_I_STATE', 'EVENT_STATE', 'EVENT_MODAL', 'EVENT_PERCEPTION', 'EVENT_REPORTING', 'EVENT_I_ACTION'], loader='list')
...
def perform_param_grid_search():
    trial_model = AutoModelForTokenClassification.from_pretrained("EMBEDDIA/est-roberta", num_labels=len(label_list), id2label=idx2label, label2id=label2idx)
    ...
    def trial_training(num_train_epochs, per_device_train_batch_size, learning_rate):
        trial_training_args = TrainingArguments(
            output_dir='est-roberta_events_main_w_class_trial_best',
            num_train_epochs=num_train_epochs,
            per_device_train_batch_size=per_device_train_batch_size,
            logging_dir='est-roberta_events_main_w_class_trial_best',
...
with open('est-roberta_events_main_w_class_best_best_params.json', 'w') as f:
    json.dump(best_parameters, f)
    
model = AutoModelForTokenClassification.from_pretrained("EMBEDDIA/est-roberta", num_labels=len(label_list), id2label=idx2label, label2id=label2idx)
model.to(device)

training_args = TrainingArguments(
    output_dir='est-roberta_events_main_w_class_param_tuning_best',
    num_train_epochs=40,
    per_device_train_batch_size=best_parameters['per_device_train_batch_size'],
    logging_dir='est-roberta_events_main_w_class_param_tuning_best',
    logging_strategy='epoch',
    save_strategy='epoch',
...
with open('est-roberta_events_main_w_class_param_tuning_best_eval.json', 'w') as f:
    json.dump(training_results, f)

# -- Saving model
trainer.save_model("./est-roberta_events_main_w_class_param_tuning_best/best/")
...
nervaluate = Evaluator(final_labels, final_predictions, tags=['EVENT_ASPECTUAL', 'EVENT_OCCURRENCE', 'EVENT_I_STATE', 'EVENT_STATE', 'EVENT_MODAL', 'EVENT_PERCEPTION', 'MAIN_EVENT_REPORTING', 'EVENT_I_ACTION'], loader='list')
results, results_by_tag, result_indices, result_indices_by_tag = nervaluate.evaluate()

# -- Saving final test results to JSON
with open('est-roberta_events_main_w_class_param_tuning_best_final_test.json', 'w') as f:
