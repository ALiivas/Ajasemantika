...
X_train, y_train = split_data(train_texts, 'gold_word_events')
X_dev, y_dev = split_data(dev_texts, 'gold_word_events')
X_test, y_test = split_data(test_texts, 'gold_word_events')
...
def compute_metrics(p):
    ...
    print(true_labels)
    assert len(true_predictions) == len(true_labels), 'Different number of predicted and true labels'
...
def perform_param_grid_search():
    trial_model = AutoModelForTokenClassification.from_pretrained("EMBEDDIA/est-roberta", num_labels=4, id2label=idx2label, label2id=label2idx)
    ...
    def trial_training(num_train_epochs, per_device_train_batch_size, learning_rate):
        trial_training_args = TrainingArguments(
            output_dir='est-roberta_events_trial_best',
            num_train_epochs=num_train_epochs,
            per_device_train_batch_size=per_device_train_batch_size,
            logging_dir='est-roberta_events_trial_best',
...
with open('est-roberta_events_best_best_params.json', 'w') as f:
    json.dump(best_parameters, f)
    
model = AutoModelForTokenClassification.from_pretrained("EMBEDDIA/est-roberta", num_labels=4, id2label=idx2label, label2id=label2idx)
model.to(device)

training_args = TrainingArguments(
    output_dir='est-roberta_events_param_tuning_best',
    num_train_epochs=40,
    per_device_train_batch_size=best_parameters['per_device_train_batch_size'],
    logging_dir='est-roberta_events_param_tuning_best',
    logging_strategy='epoch',
    save_strategy='epoch',
...
with open('est-roberta_events_param_tuning_best_eval.json', 'w') as f:
    json.dump(training_results, f)

# -- Saving model
trainer.save_model("./est-roberta_events_param_tuning_best/best/")
...
with open('est-roberta_events_param_tuning_best_final_test.json', 'w') as f:
