...
def predict_on_tempf(model, tokenizer, layer):
    ...
    for i in range(len(all_preds)):
        if all_preds[i] != labels[i]:
            with open("Est-RoBERTa_predict_on_tempf_values_w_classes_no_horisont.txt", "a", encoding="utf-8") as f:
                f.write(filenames[i]+"\n")
    ...
    with open('masters_thesis/est-roberta_events_w_class_param_tuning_pred_tempf_no_horisont.json', 'w') as f: 
        json.dump([results, results_by_tag, result_indices, result_indices_by_tag], f)  
    
tokenizer = AutoTokenizer.from_pretrained('./masters_thesis/est-roberta_events_w_class_param_tuning_best/best')
model = AutoModelForTokenClassification.from_pretrained('./masters_thesis/est-roberta_events_w_class_param_tuning_best/best')
