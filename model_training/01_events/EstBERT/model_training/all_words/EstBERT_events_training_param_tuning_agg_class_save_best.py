...
def perform_param_grid_search():
    trial_model = AutoModelForTokenClassification.from_pretrained("tartuNLP/EstBERT", num_labels=9, id2label=idx2label, label2id=label2idx)
    ...
    def trial_training(num_train_epochs, per_device_train_batch_size, learning_rate):
        trial_training_args = TrainingArguments(
            output_dir='estbert_events_w_agg_class_trial_best',
            num_train_epochs=num_train_epochs,
            per_device_train_batch_size=per_device_train_batch_size,
            logging_dir='estbert_events_w_agg_class_trial_best',
...
with open('estbert_events_w_agg_class_best_best_params.json', 'w') as f:
    json.dump(best_parameters, f)
    
model = AutoModelForTokenClassification.from_pretrained("tartuNLP/EstBERT", num_labels=9, id2label=idx2label, label2id=label2idx)
model.to(device)

training_args = TrainingArguments(
    output_dir='estbert_events_w_agg_class_param_tuning_best',
    num_train_epochs=40,
    per_device_train_batch_size=best_parameters['per_device_train_batch_size'],
    logging_dir='estbert_events_w_agg_class_param_tuning_best',
    logging_strategy='epoch',
    save_strategy='epoch',
...
with open('estbert_events_w_agg_class_param_tuning_best_eval.json', 'w') as f:
    json.dump(training_results, f)

# -- Saving model
trainer.save_model("./estbert_events_w_agg_class_param_tuning_best/best/")
...
with open('estbert_events_w_agg_class_param_tuning_best_final_test.json', 'w') as f:
