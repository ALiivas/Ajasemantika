...
def predict_on_tempf(model, tokenizer, layer):
    ...
    for input in inputs_list:
        ...
        for i in range(len(tokens)):
            if tokens[i].startswith("##"):
                ...
                cleaned_tokens[-1] = cleaned_tokens[-1] + tokens[i][2:]
            elif tokens[i] == '[CLS]' or tokens[i] == '[SEP]':
                continue
            else:
    ...
    for i in range(len(all_preds)):
        if all_preds[i] != labels[i]:
            with open("EstBERT_predict_on_tempf_values_w_classes_no_horisont.txt", "a", encoding="utf-8") as f:
                f.write(filenames[i]+"\n")
    ...
    nervaluate = Evaluator(labels, all_preds, tags=['EVENT_ASPECTUAL', 'EVENT_OCCURRENCE', 'EVENT_I_STATE', 'EVENT_STATE', 'EVENT_MODAL', 'EVENT_PERCEPTION', 'EVENT_REPORTING', 'EVENT_I_ACTION'], loader='list')
    results, results_by_tag, result_indices, result_indices_by_tag = nervaluate.evaluate()
    
    # -- Saving evaluation results to JSON
    with open('masters_thesis/estbert_events_w_class_param_tuning_pred_tempf_no_horisont.json', 'w') as f: 
        json.dump([results, results_by_tag, result_indices, result_indices_by_tag], f)  
    
tokenizer = AutoTokenizer.from_pretrained('./masters_thesis/estbert_events_w_class_param_tuning_best/best')
model = AutoModelForTokenClassification.from_pretrained('./masters_thesis/estbert_events_w_class_param_tuning_best/best')

predict_on_tempf(model, tokenizer, 'gold_word_events_w_classes')