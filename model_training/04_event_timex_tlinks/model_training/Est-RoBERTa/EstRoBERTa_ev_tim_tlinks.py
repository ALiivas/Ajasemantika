...
def run_pipeline(clf, save_path, clf_name, embed_type, X_train, y_train, X_test, y_test):
    ...
    if clf_name != "dummy":
        ...
        pickle.dump(clf, open(f'estroberta_ev_timex_models/estroberta_{clf_name}_{embed_type}.pkl', 'wb'))
    ...
    with open(f'{save_path}/estroberta_ev_timex_accuracy.txt', 'a') as f:
...
# peasõna vektorid
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'main_concat', X_train_main_concat, y_train_concat, X_test_main_concat, y_test_concat)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'main_concat', X_train_main_concat, y_train_concat, X_test_main_concat, y_test_concat)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'main_concat', X_train_main_concat, y_train_concat, X_test_main_concat, y_test_concat)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'main_concat', X_train_main_concat, [labels2idx[label] for label in y_train_concat], X_test_main_concat, [labels2idx[label] for label in y_test_concat])
run_pipeline(dummy_clf, 'estroberta_ev_timex_res', 'dummy', 'main_concat', X_train_main_concat, y_train_concat, X_test_main_concat, y_test_concat)

# fraasi vektorite aritmeetiline keskmine
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'mean_concat', X_train_mean_concat, y_train_concat, X_test_mean_concat, y_test_concat)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'mean_concat', X_train_mean_concat, y_train_concat, X_test_mean_concat, y_test_concat)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'mean_concat', X_train_mean_concat, y_train_concat, X_test_mean_concat, y_test_concat)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'mean_concat', X_train_mean_concat, [labels2idx[label] for label in y_train_concat], X_test_mean_concat, [labels2idx[label] for label in y_test_concat])
run_pipeline(dummy_clf, 'estroberta_ev_timex_res', 'dummy', 'mean_concat', X_train_mean_concat, y_train_concat, X_test_mean_concat, y_test_concat)

# nelja viimase kihi summa
# peasõna vektorid
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'main_add', X_train_main_add, y_train_add, X_test_main_add, y_test_add)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'main_add', X_train_main_add, y_train_add, X_test_main_add, y_test_add)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'main_add', X_train_main_add, y_train_add, X_test_main_add, y_test_add)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'main_add', X_train_main_add, [labels2idx[label] for label in y_train_add], X_test_main_add, [labels2idx[label] for label in y_test_add])
run_pipeline(dummy_clf, 'estroberta_ev_timex_res', 'dummy', 'main_add', X_train_main_add, y_train_add, X_test_main_add, y_test_add)

# fraasi vektorite aritmeetiline keskmine
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'mean_add', X_train_mean_add, y_train_add, X_test_mean_add, y_test_add)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'mean_add', X_train_mean_add, y_train_add, X_test_mean_add, y_test_add)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'mean_add', X_train_mean_add, y_train_add, X_test_mean_add, y_test_add)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'mean_add', X_train_mean_add, [labels2idx[label] for label in y_train_add], X_test_mean_add, [labels2idx[label] for label in y_test_add])
run_pipeline(dummy_clf, 'estroberta_ev_timex_res', 'dummy', 'mean_add', X_train_mean_add, y_train_add, X_test_mean_add, y_test_add)

# eelviimane kiht
# peasõna vektorid
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'main_penult', X_train_main_penult, y_train_penult, X_test_main_penult, y_test_penult)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'main_penult', X_train_main_penult, y_train_penult, X_test_main_penult, y_test_penult)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'main_penult', X_train_main_penult, y_train_penult, X_test_main_penult, y_test_penult)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'main_penult', X_train_main_penult, [labels2idx[label] for label in y_train_penult], X_test_main_penult, [labels2idx[label] for label in y_test_penult])
run_pipeline(dummy_clf, 'estroberta_ev_timex_res', 'dummy', 'main_penult', X_train_main_penult, y_train_penult, X_test_main_penult, y_test_penult)

# fraasi vektorite aritmeetiline keskmine
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'mean_penult', X_train_mean_penult, y_train_penult, X_test_mean_penult, y_test_penult)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'mean_penult', X_train_mean_penult, y_train_penult, X_test_mean_penult, y_test_penult)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'mean_penult', X_train_mean_penult, y_train_penult, X_test_mean_penult, y_test_penult)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'mean_penult', X_train_mean_penult, [labels2idx[label] for label in y_train_penult], X_test_mean_penult, [labels2idx[label] for label in y_test_penult])

# viimane kiht
# peasõna vektorid
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'main_last', X_train_main_last, y_train_last, X_test_main_last, y_test_last)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'main_last', X_train_main_last, y_train_last, X_test_main_last, y_test_last)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'main_last', X_train_main_last, y_train_last, X_test_main_last, y_test_last)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'main_last', X_train_main_last, [labels2idx[label] for label in y_train_last], X_test_main_last, [labels2idx[label] for label in y_test_last])
run_pipeline(dummy_clf, 'estroberta_ev_timex_res', 'dummy', 'main_last', X_train_main_last, y_train_last, X_test_main_last, y_test_last)

# fraasi vektorite aritmeetiline keskmine
run_pipeline(svc_clf, 'estroberta_ev_timex_res', 'SVC', 'mean_last', X_train_mean_last, y_train_last, X_test_mean_last, y_test_last)
run_pipeline(rf_clf, 'estroberta_ev_timex_res', 'RF', 'mean_last', X_train_mean_last, y_train_last, X_test_mean_last, y_test_last)
run_pipeline(mlp_clf, 'estroberta_ev_timex_res', 'MLP', 'mean_last', X_train_mean_last, y_train_last, X_test_mean_last, y_test_last)
run_pipeline(xgb_clf, 'estroberta_ev_timex_res', 'XGB', 'mean_last', X_train_mean_last, [labels2idx[label] for label in y_train_last], X_test_mean_last, [labels2idx[label] for label in y_test_last])
run_pipeline(dummy_clf, 'estroberta_ev_timex_res', 'dummy', 'mean_last', X_train_mean_last, y_train_last, X_test_mean_last, y_test_last)
